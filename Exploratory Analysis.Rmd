...
tidy_bing_custom_stop %>%
  ...
  group_by(Professionalization, n, Elected) %>%
  ...
  ggplot(aes(x = Professionalization, y = total_sentiment, fill = Elected)) +
  geom_col(position = "dodge") +
  labs(x = "Level of Professionalization", y = "Net Sentiment")
  ...
  mutate(sentiment = ifelse(sentiment == "negative", -1, 1),
         culture = ifelse(Moralistic  == 1, "Moralistic",
                          ifelse(Traditionalistic == 1, "Traditionalistic",
                                 "Individualistic"))) %>%
  add_count(culture) %>% 
  group_by(culture, n, Elected) %>%
  summarize(total_sentiment = sum(sentiment)) %>%
  ungroup() %>%
  # mutate(proportional_sentiment = total_sentiment / n) %>%
  filter(!(is.na(culture))) %>%
  # ggplot(aes(x = culture, y = proportional_sentiment)) +
  ggplot(aes(x = culture, y = total_sentiment, fill = Elected)) +
  geom_col(position = "dodge") +
  labs(x = "Political Culture", y = "Net Sentiment")
